...
@section('page_title', __('user.edit_user'))
@section('content')
<div class="container mt-2 box">
    ...
    @endif
    <form action="{{ route('users.update',$user->id) }}" method="POST" enctype="multipart/form-data" id="myform">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group{{ $errors->has('role') ? ' has-error' : '' }}">
                    ...
                    <select name="role" {{ \Illuminate\Support\Facades\Auth::id() == $user->id  ? 'disabled="disabled"':'' }}>
                        <option value="0" {{ 0 == $user->role  ? 'selected="selected"':'' }}>{{__('user.member')}}</option>
